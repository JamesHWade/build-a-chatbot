...
source("helper.R")
source("pdf-to-index.R")
source("get_context.R")

ui <- page_fillable(
  useWaiter(),
  waiterOnBusy(html = spin_3circles(), color = transparent(0.5)),
...
server <- function(input, output, session) {
  ...
  rv$api_key <- reactive(input$api_key)
  ingest_pdf_server("pdfs", rv)
  observe({
    if (input$api_key == "") {
  ...
  observe({
    ...
    prompt <- get_context(input$prompt, rv$pdf_index, k = 4)
